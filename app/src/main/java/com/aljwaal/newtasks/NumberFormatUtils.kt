...
object NumberFormatUtils {
    ...
    /** يحوّل ٠١٢٣ و۰۱۲۳ إلى الأرقام اللاتينية المطلوبة 0123. */
    ...
    fun formatDate(millis: Long, includeYear: Boolean = true): String {
        val calendar = Calendar.getInstance().apply { timeInMillis = millis }
        val day = number(calendar.get(Calendar.DAY_OF_MONTH))
        val month = months[calendar.get(Calendar.MONTH)]
        val year = number(calendar.get(Calendar.YEAR))
    ...
    fun monthTitle(year: Int, month: Int): String = "${months[month]} ${number(year)}"
    ...
    /** لا يفترض أن اليوم 24 ساعة، لذلك يبقى صحيحًا في أيام التوقيت الصيفي والشتوي. */
